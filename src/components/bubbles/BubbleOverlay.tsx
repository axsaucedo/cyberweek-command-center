...
function BubbleOverlayInner({ orders, intensity }: Props) {
  ...
  const loop = useCallback(() => {
    ...
    while (pendingRef.current.length > 0 && cardsRef.current.length < MAX_CARDS) {
      ...
      const cardW = isHighValue ? 52 + Math.random() * 16 : 36 + Math.random() * 12;
    ...
    while (i--) {
      ...
      c.x += c.vx * dt;
      c.alpha = Math.min(1, c.life / (c.maxLife * 0.25));
    }
    ...
    for (const c of cardsRef.current) {
      ...
      if (c.isHighValue && c.width > 40) {
        ctx.font = `bold ${Math.max(8, c.width * 0.17)}px system-ui`;
        ctx.fillStyle = '#fff';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.globalAlpha = c.alpha * 0.7;
        ctx.fillText(`$${Math.round(c.amount)}`, c.x, c.y + c.height / 2 + 3);
      }
    }
